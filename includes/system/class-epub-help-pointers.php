 ...
 */
class EPUB_Help_Pointers {

	/**
	 * Constructor.
	 *
	 * @param $plugin_activated_first_time - whether to show help about new plugin rather than new update
	 */
	public function __construct( $plugin_activated_first_time ) {
		...
		$help_function = $plugin_activated_first_time ? 'setup_pointers_for_new_install' : 'setup_pointers_for_update';
	...
	public function setup_pointers_for_new_install() {
		...
		$pointers = array(
			'pointers' => array(
				'posts_lookup' => array(
					'target'       => "#wp-admin-bar-epub-post-links",
					'next'         => 'pages_lookup',
					'next_trigger' => array(
						/* 'target' => "#epub_help_later",
						'event'  => 'change blur click' */
					),
					'options'      => array(
						'content'  => 	'<h3>' . esc_html( 'Quick Tour of Enhanced Publishing' ) . '</h3>' .
				...
				'pages_lookup' => array(
					'target'       => "#wp-admin-bar-epub-page-links",
					'next'         => 'cpts_lookup',
					'next_trigger' => array(),
					'options'      => array(
						'content'  => 	'<h3>' . esc_html( 'Quick Tour of Enhanced Publishing' ) . '</h3>' .
				...
				'cpts_lookup' => array(
					'target'       => "#wp-admin-bar-epub-cpt-links",
 ...
 */
function epub_print_js() {
	global $epub_pointers;

	$js = "jQuery( function( $ ) {
				var epub_pointers = {$epub_pointers};

				setTimeout( init_epub_pointers, 1000 );

				function init_epub_pointers() {
					$.each( epub_pointers.pointers, function( i ) {
						epub_show_pointer( i );
						return false;
					});
				}

				function epub_show_pointer( id ) {
					var pointer = epub_pointers.pointers[id];
					var options = $.extend( pointer.options, {
						close: function() {
							if ( pointer.next ) {
								epub_show_pointer( pointer.next );
							}
						}
					} );
...
add_action( 'admin_footer', 'epub_print_js' );
